...
public class HomeActivity extends ListActivity {
	...
	private void createDialog(final View v, int flag) {
		...
		case MESSAGE_OPTIONS:
			...
			alert.setItems(R.array.message_details_array,
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		switch (item.getItemId()) {
			...
			// Included for sake of demo purposes
			case R.id.menu_receive:
				SharedPreferences sps = PreferenceManager.getDefaultSharedPreferences(getBaseContext());
				Editor ed = sps.edit();
				
				boolean temp = sps.getBoolean(Settings.KEY_TOGGLE, Settings.MODE_SEND);
				ed.putBoolean(Settings.KEY_TOGGLE, !temp);
				ed.commit();
				
				if (sps.getBoolean(Settings.KEY_TOGGLE, true))
					Toast.makeText(getApplicationContext(), "Mode: Sending", Toast.LENGTH_SHORT).show();
				else
					Toast.makeText(getApplicationContext(), "Mode: Receiving", Toast.LENGTH_SHORT).show();
				return true;
			}
	...
	public void onSend(View v) {
		...
		if (message.length() > 0) {
			...
			if (b) { // Sending
				...
				Message m = new Message(false, message, name, getLocalIpAddress(), getPriority());
				SystemInterface.receive(this, m);
			}
			scrollListViewToBottom();
		}
